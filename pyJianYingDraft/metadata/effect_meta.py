...
class EffectParam:
    ...
    def __init__(
        self, name: str, default_value: float, min_value: float, max_value: float
    ):
        self.name = name
...
class EffectMeta:
    ...
    def parse_params(
        self, params: Optional[List[Optional[float]]]
    ) -> List[EffectParamInstance]:
            ...
            if i < len(params):
                ...
                if input_v is not None:
                    if input_v < 0 or input_v > 100:
                        raise ValueError(
                            "Invalid parameter value %f within %s"
                            % (input_v, str(param))
                        )
                    val = (
                        param.min_value
                        + (param.max_value - param.min_value) * input_v / 100.0
                    )  # 从0~100映射到实际值
