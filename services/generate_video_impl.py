...
async def generate_video_impl(
    ...
    if framerate and framerate not in valid_framerates:
        result["error"] = (
            f"Invalid framerate '{framerate}'. Must be one of: {', '.join(valid_framerates)}"
        )
        return result
    ...
    try:
        ...
        materials = (
            draft_content.get("materials") if isinstance(draft_content, dict) else None
        )
        material_count = 0
        if isinstance(materials, dict):
            for material_group in materials.values():
                if isinstance(material_group, list):
                    ...
                    material_count += sum(
                        len(sub_group)
                        for sub_group in material_group.values()
                        if isinstance(sub_group, list)
                    )

        if material_count == 0:
            logger.warning(
                f"Draft {draft_id} contains no materials. Aborting video generation dispatch."
            )
            result["error"] = (
