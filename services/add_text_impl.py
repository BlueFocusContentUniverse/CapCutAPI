...
async def add_text_impl(
    ...
    text_border = (
        draft.Text_border(alpha=border_alpha, color=rgb_border_color, width=border_width)
        if border_width > 0
        else None
    )

    text_background = (
        draft.Text_background(
    ...
    text_bubble = (
        TextBubble(effect_id=bubble_effect_id, resource_id=bubble_resource_id)
        if bubble_effect_id and bubble_resource_id
        else None
    )

    text_effect = TextEffect(effect_id=effect_effect_id) if effect_effect_id else None
    ...
    if text_styles:
        for style_range in text_styles:
            if (
                ...
                raise ValueError(
                    f"Invalid text range: [{style_range.start}, {style_range.end}), text length: {len(text)}"
                )
            text_segment.add_text_style(style_range)

    # Add bubble and effect
    if text_bubble:
    ...
    if intro_animation:
        try:
            animation_type = (
                getattr(draft.CapCutTextIntro, intro_animation)
                if IS_CAPCUT_ENV
                else getattr(draft.TextIntro, intro_animation)
            )
            text_segment.add_animation(animation_type, int(intro_duration * 1000000))
        except Exception as e:
            logger.warning(f"Unsupported intro animation: {intro_animation}, ignored: {e}")

    # Add outro animation
    if outro_animation:
        try:
            animation_type = (
                getattr(draft.CapCutTextOutro, outro_animation)
                if IS_CAPCUT_ENV
                else getattr(draft.TextOutro, outro_animation)
            )
            text_segment.add_animation(animation_type, int(outro_duration * 1000000))
        except Exception as e:
            logger.warning(f"Unsupported outro animation: {outro_animation}, ignored: {e}")
