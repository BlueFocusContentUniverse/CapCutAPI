...
def save_draft_impl(
    ...
    2. 获取草稿内容
    3. 创建归档记录
    4. 发送任务到 draft_archive_notice 项目的 Celery 队列
    
    Returns:
        包含 success, archive_id 等字段的字典
    """
    ...
    try:
        ...
        if draft_version is not None:
            ...
            # 获取当前版本
            result = get_from_cache_with_version(draft_id)
            if result is None:
                raise Exception(f"Draft {draft_id} does not exist in cache (memory or PostgreSQL)")
            script, actual_version = result
        
        # 将 Script 对象转换为字典（JSON）
